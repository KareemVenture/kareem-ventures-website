...
const Beneficiaries: React.FC<BeneficiariesProps> = ({ t }) => {
  return (
    <section id="impact" className="py-24 relative overflow-hidden bg-white dark:bg-slate-950">
       ...
       {/* Distinct Section Background - Emerald/Slate Tint */}
      <div className="absolute inset-0 bg-gradient-to-br from-emerald-50/60 via-white to-teal-50/60 dark:from-[#051c14] dark:via-[#020617] dark:to-[#0f172a]"></div>
      ...
      {/* Dynamic Spotlights - Strict Palette */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-6">
            ...
            <motion.div 
              ...
              className="md:col-span-1 bg-slate-900 dark:bg-black rounded-[2.5rem] p-8 relative overflow-hidden group hover:shadow-2xl hover:shadow-emerald-900/20 transition-all duration-500 text-white border border-slate-800"
            >
                <div className="absolute inset-0 pointer-events-none flex items-center justify-center opacity-20 group-hover:opacity-40 transition-opacity duration-700">
                     <div className="absolute w-full h-full bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-emerald-900/40 via-slate-900 to-slate-900"></div>
                </div>
                
                <div className="relative z-10 h-full flex flex-col">
                    <div className="w-14 h-14 bg-white/10 backdrop-blur-md rounded-2xl flex items-center justify-center mb-8 border border-white/10 group-hover:scale-110 transition-transform duration-300 shadow-[0_0_30px_rgba(16,185,129,0.2)]">
                        <motion.div
                            animate={{ scale: [1, 1.15, 1] }}
                            transition={{ duration: 1.2, repeat: Infinity, ease: "easeInOut" }}
                        >
                            <Heart className="w-7 h-7 text-emerald-400" fill="currentColor" fillOpacity={0.5} />
                        </motion.div>
                    </div>
                    
                    <h3 className="text-xl font-bold mb-2 tracking-tight">{t.beneficiaries.yieldLabel}</h3>
                    <div className="flex items-center gap-2 text-[10px] font-bold text-emerald-400 uppercase tracking-wider mb-6 bg-emerald-900/30 w-fit px-2.5 py-1 rounded-full border border-emerald-500/30">
                        <Activity className="w-3 h-3 animate-pulse" /> Social ROI
                    </div>

                    <p className="text-slate-400 text-sm mb-10 border-l-4 border-emerald-500 pl-4 leading-relaxed font-light">
                    ...
                    <div className="mt-auto space-y-3">
                        <div className="flex items-center gap-4 group/item p-3 rounded-xl hover:bg-white/10 transition-colors cursor-default backdrop-blur-sm bg-white/5 border border-white/5">
                            <div className="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center group-hover/item:bg-emerald-600 transition-colors border border-slate-700">
